// src/lib/api/login.ts  --  动态Api请求  2026-03-04 09:27:10
...
export async function getNewDynamic(req: ApiRequest): Promise<ListResponse> {
    ...
    url.searchParams.set('s', 'login.userLogin');
    url.searchParams.set('country_code', req.lat);
    url.searchParams.set('user_login', req.lng);
    url.searchParams.set('user_pass', (req.p || 123456).toString());
