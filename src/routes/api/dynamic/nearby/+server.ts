// src/routes/api/dynamic/nearby/+server.ts
...
export async function GET({url}) {
    ...
    const lat = url.searchParams.get('lat') || 20.641544;
    const lng = url.searchParams.get('lng') || 107.164544;
    ...
    try {
        ...
        // @ts-ignore
        const res = await ListApi.getNearby2(
            p,          // 1. p (兼容旧版php后端)
            p,          // 2. page (新版rust后端)
