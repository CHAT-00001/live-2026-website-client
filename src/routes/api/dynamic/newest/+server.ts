// src/routes/api/dynamic/newest/+server.ts
...
export async function GET({url}) {
    ...
    const lat = url.searchParams.get('lat') || 20.16444;
    const lng = url.searchParams.get('lng') || 108.16245;
    ...
    try {
        ...
        // @ts-ignore
        const res = await ListApi.getNewest(
            p,          // 1. p (兼容旧版php后端)
            p,          // 2. page (新版rust后端)
